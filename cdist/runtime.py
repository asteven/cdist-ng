...
class Runtime(dict):
    ...
    def from_dir(cls, path):
        """Creates a cdist runtime environment instance from an existing
        directory.
        """
        obj = cls()
    ...
    def to_dir(self, path):
        """Store this runtime environment instance in a directory for use
        by shell scripts.
        """
        cconfig.to_dir(path, self, schema=self.schema)

    def __init__(self, remote_cache_dir=None, exec_path=None):
        super().__init__(cconfig.from_schema(self.schema))
        self['remote-cache-dir'] = remote_cache_dir
        self['exec-path'] = exec_path

    def add_conf_dir(self, conf_dir):
        """Add a conf dir to the runtime environment.
